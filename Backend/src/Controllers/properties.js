...
const createPropertie = async (req, res) => {
    ...
    try {
        ...
        const result = await Propertie.createPropertie(
            linkImage,
            linkDoc,
            description,
            type,
...
const getPropertyMetadata = async (req, res) => {
    //Pega as infos da requisição
    const { id } = req.params;
    ...
    try {
        //Tratamento das respostas do método da classe
        const result = await Propertie.getPropertyMetadata(id);
        res.send(result);
    } catch (err) {
        res.status(500).send(err.message);
    }
...
const updatePropertie = async (req, res) => {
    ...
    try {
        //Tratamento das respostas do método da classe
        const result = await Propertie.updatePropertie(
            id_prop,
            title,
            currentPrice,
            type,
